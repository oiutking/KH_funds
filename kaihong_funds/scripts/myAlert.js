(function() {
	$.extend({
		myAlert: function(options) {//参数格式{title:'Title',message:'message',callback:function(){alert('callback')}}or"需要提示的话"
			...
			$('body').append('<div class="myModa"><div class="myAlertBox"  style="margin-top:' + top + 'px"><h6>' + option.title + '</h6><p>' + option.message + '</p><div class="btn_alert sure ">确定</div></div></div>');
			$('.btn_alert.sure').click(function(){
				$('.myModa').remove();
				option.callback();
			})
		},
